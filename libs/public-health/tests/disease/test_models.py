...
#########################
# Data-source threading #
#########################


def test_defaults_resolve_to_artifact_keys():
    model = models.SI(CAUSE)
    infected = _state(model, DiseaseState)
    ...
    # No cause-specific mortality source provided -> model loads it from artifact.
    assert model._csmr_source is None


def test_supplied_sources_are_threaded_through():
    prevalence = 0.1
    incidence = 0.2
    csmr = 0.3
    model = models.SI(
    ...
    assert infected._prevalence_source == prevalence
    assert _rate_transition(healthy).transition_rate == incidence
    assert model._csmr_source == csmr

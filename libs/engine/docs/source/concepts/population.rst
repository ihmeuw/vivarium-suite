...
Deferring the append this way keeps the existing simulants' columns untouched while
the new ones are being filled in, so a column is never widened with null rows for
simulants whose values have not been computed yet. Reads during a creation pass are
served from whichever frame holds the requested simulants, so an initializer still
sees the values that earlier initializers produced. A column whose initializer has
not run yet reads as null, as it did when the state table was padded directly.

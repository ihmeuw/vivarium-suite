...
from vivarium.engine.framework.artifact.artifact import (
    Artifact,
    ArtifactException,
    _parse_draw_filters,
    _to_tree,
...
def hdf_mock(mocker: pytest_mock.MockFixture, keys_mock: list[str]) -> MagicMock:
    mock = mocker.patch("vivarium.engine.framework.artifact.artifact.hdf")
...
def test_artifact_creation(
    ...
    assert a._cache == {}
    assert a.keys == keys_mock
    hdf_mock.load.assert_called_with(artifact_path, "metadata.keyspace", None, None)
...
def test_artifact_load_key_has_no_data(hdf_mock: MagicMock, artifact_path: Path) -> None:
    ...
    hdf_mock.load.assert_called_with(artifact_path, key, filter_terms, ["draw_10", "value"])
    assert a._cache == {}
...
def test_artifact_load(
    ...
    for key in keys_without_metadata:
        ...
        hdf_mock.load.assert_called_with(

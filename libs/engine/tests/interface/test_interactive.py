...
class TestObserverExclusion:
    ...
    def test_excluded_from_a_model_specification(self) -> None:
        """The spec path builds components through the parser, not from a list."""
        sim = InteractiveContext(
            get_model_specification_path(), include_observers=True, setup=False
        )
        assert self._observers(sim) == ["deaths_observer", "ylls_observer"]
    ...
    def test_empty_results_say_why(self, caplog: LogCaptureFixture) -> None:
        ...
        assert "include_observers=True" in caplog.text
    ...
    def test_empty_results_say_why_only_once(self, caplog: LogCaptureFixture) -> None:
        ...
        assert caplog.text.count("include_observers=True") == 1

    def test_no_warning_when_observers_were_requested(
        ...
        assert "include_observers=True" not in caplog.text

    def test_leave_no_stratification_configuration_behind(self) -> None:

...
- API: the interpolation validators (``check_data_complete``, ``validate_parameters``, ``validate_call_data``) take continuous-parameter records instead of ``(name, start, end)`` tuples.
- API: ``has_named_row_index`` moved from ``vivarium.engine.framework.lookup.interpolation`` to ``vivarium.engine.types``.

...
class TestWorkflowSubcommand:
    ...
    def test_workflow_cli_overrides_config(
        self,
        tmp_path: Path,
        cli_args: list[str],
        expected_overrides: dict[str, str],
    ) -> None:
        """CLI flags override corresponding values from config file.
    ...
    def test_workflow_missing_config_flag_errors(self) -> None:
        ...
        The -c/--config flag is required for the workflow subcommand.
        Click should raise an error if it's missing.
        """
        cli_runner = CliRunner()
        result = cli_runner.invoke(psimulate, ["workflow"])
        assert result.exit_code != 0

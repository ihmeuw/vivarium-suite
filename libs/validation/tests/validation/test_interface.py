...
from vivarium.validation.data_transformation.measures import MeasureMapper, RatioMeasure
...
def test__get_age_groups_art(sim_result_dir: Path, mocker: MockFixture) -> None:
    ...
    mocker.patch(
        "vivarium.validation.data_loader.Artifact.load",
...
def test_metadata(sim_result_dir: Path, mocker: MockFixture) -> None:
    ...
    mocker.patch(
        "vivarium.validation.interface.Path.name",
        "2025_01_01_00_00_00",
    )
    artifact_mtime = 1735718340  # Dec 31 23:59 2024 PST / Jan 01 07:59 2025 UTC
    mocker.patch(
        "vivarium.validation.interface.os.path.getmtime",
...
def test_plot_comparison(sim_result_dir: Path, mocker: MockFixture) -> None:
    ...
    mock_plot_comparison = mocker.patch(
        "vivarium.validation.visualization.plot_utils.plot_comparison",
...
def test_figures_to_base64_dict(sim_result_dir: Path, mocker: MockFixture) -> None:
    ...
    mock_close = mocker.patch("vivarium.validation.interface.plt.close")

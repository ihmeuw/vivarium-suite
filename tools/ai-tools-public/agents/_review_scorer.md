---
name: _review_scorer
description: "Use when: independently scoring a single code-review finding for confidence (0-100) against the verbatim rubric, so low-confidence findings can be filtered out before synthesis. Spawned one-per-finding by simsci:_review-core."
tools:
  - Read
  - Grep
  - Glob

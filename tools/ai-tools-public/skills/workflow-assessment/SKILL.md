...
allowed-tools: Read, Grep, Glob, Agent(simsci:_trace_extractor)
---
...
3. **Fingerprint without opening transcripts.** The `.jsonl`'s **mtime** is when
   ...
   their `agentType`s (e.g. `simsci:_split_proposer` ⟹ `/simsci:commit-splitter`). No `subagents/`
...
dispatch `simsci:_trace_extractor` **in a single message** — one per transcript (main +

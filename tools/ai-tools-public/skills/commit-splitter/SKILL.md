...
Delegate the grouping to the `viv-public:_split_proposer` specialist agent. It has read-only access to the working tree and returns a structured plan — commit groups (subject, rationale, files, dependencies), a PR partition, and any inseparable hunks. Keeping the file-by-file reading inside the subagent keeps the main context clean.
...
When the plan calls for multiple PRs, each PR needs its own branch. Follow your team's branch-naming, ticket-linkage, and PR conventions — if an installed skill covers them (e.g. a team conventions skill), invoke it and follow it rather than re-deriving them here. Typical shape:

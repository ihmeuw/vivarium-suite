...
description: Propagate an adapted copy of a reference file or directory across several targets — packages or paths in the reference's own repo, and/or external repos — in parallel, then file one draft PR per repo. Use when the user wants to roll a piece of boilerplate or config out to many places at once. Not for a single-target edit (just edit it) and not when the change needs bespoke per-target design rather than adaptation of a shared reference.
---
...
targets are a mix of local packages or directories (in whatever repository the
reference lives in — a monorepo or not; discover its layout from the repo) and
...
- **Target list** → classify each entry:
  - a package or directory in the reference's own repo, by name or path
    (`config-tree`, `packages/engine`) → `local` substrate;
...
Brief each worker with: its single `target` (for a local target, its
...
- **Local workers** each run in their own **isolated git worktree** — spawn
...
— is worth the cost. Then converge by substrate: one PR for the reference's
own repo (integrate every local worker's worktree onto a single branch of it;
their subtrees are disjoint, so they union cleanly), and one PR for each
external repo. For **small, uniform,
...
any other target; integrate its worktree onto the same local-repo branch
...
with reasons; and the PR set you intend to file (one PR for the reference's
repo + one per external repo, each a **draft**). Then take **one bulk approval** to proceed.
...
one, and open every PR as a **draft**. File one PR for the reference's repo
from the integrated local branch and one PR per external repo. Offer to
announce the PRs in your team channel.

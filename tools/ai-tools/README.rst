...
``/viv:code-reviewer`` and ``/viv:model-regression-debugger``. These run
...
- The ``/viv:code-reviewer`` and ``/viv:model-regression-debugger`` slash
  command bodies (running in the main session) gather PR/repo context
  through the GitHub MCP server (a plugin dependency; see the
  ``plugin-setup`` skill), falling back to read-only git/``gh`` commands

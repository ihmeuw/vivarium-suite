...
2. **Draw the scope line.** Ask the user which findings they are addressing in
   ...
   stop. **If the caller already drew this line** — `/simsci:pr-prep`'s
   disposition gate and `simsci:_finalize-core` both say so explicitly when they
   hand over a pre-scoped set — take it as given and go straight to step 3.
   Asking again re-opens a decision the user already made.

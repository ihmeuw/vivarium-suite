...
Both files live at `vivarium_build_utils/resources/makefiles/`. That means **the same set of `make` targets is available in essentially every vivarium library**. When you see `make <target>` in a vivarium context, look here.
...
3. Read `vivarium_build_utils/resources/makefiles/base.mk` and `test.mk` directly — those are the source of truth and may have changed since this skill was written.
